...
func CompileInstr(s string) (string, error) {
	...
	args := make([]*big.Int, 6)
	for i, val := range tokens[1:len(tokens)] { //最多可處理除了OpCode以外的6個參數
		num := new(big.Int)
		num.SetString(val, 0)
		args[i] = num
	}
...
func Instr(instr string) (int, []string, error) {
	...
	for i := 0; i < 7; i++ {
		...
		// 上面這行公式就是用以下四行程式碼完成，而不能直接用上面這麼一行來完成的理由也很好理解，就是我們的val，也就是程式碼中的變數base，是一個*big.Int，用來處理超大整數。
		// 公式的目的為解碼CompileInstr所產生的編碼指令字串，將其還原為7個指令。
		exp := BigPow(256, i)
		num := new(big.Int)
		num.Div(base, exp)
	...
	op, _ := strconv.Atoi(args[0]) // 轉換成int類型
